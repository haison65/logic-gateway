...
WORKDIR /
COPY --from=build /out/http2gw /http2gw
COPY configs/http2gw.docker.yaml /configs/http2gw.yaml
...
WORKDIR /
COPY --from=build /out/logic /logic
COPY configs/logic.docker.yaml /configs/logic.yaml
USER nonroot:nonroot
EXPOSE 9100/udp
ENTRYPOINT ["/logic"]
